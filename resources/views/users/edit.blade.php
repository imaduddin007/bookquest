...
@section('content')
    <div class="col-md-8">
        ...
        @endif
        <form enctype="multipart/form-data" class="bg-white shadow-sm p-3 form-group"
            ...
            <input {{ $user->status == 'ACTIVE' ? 'checked' : '' }} value="ACTIVE" type="radio" class="form-check-input"
                id="active" name="status"> <label for="active" class="form-check-label">Active</label>
            </div>
            <div class="form-check form-check-inline">
            <input {{ $user->status == 'INACTIVE' ? 'checked' : '' }} value="INACTIVE" type="radio" class="form-check-input"
                id="inactive" name="status"> <label for="inactive" class="form-check-label">Inactive</label>
            </div>
            <br><br>
            ...
            <input type="checkbox" {{ in_array('ADMIN', json_decode($user->roles)) ? 'checked' : '' }} name="roles[]"
                class="form-check-input {{ $errors->first('roles') ? 'is-invalid' : '' }}" id="ADMIN" value="ADMIN">
            ...
            <input type="checkbox" {{ in_array('STAFF', json_decode($user->roles)) ? 'checked' : '' }} name="roles[]"
                class="form-check-input {{ $errors->first('roles') ? 'is-invalid' : '' }}" id="STAFF" value="STAFF">
            ...
            <input type="checkbox" {{ in_array('CUSTOMER', json_decode($user->roles)) ? 'checked' : '' }} name="roles[]"
                class="form-check-input {{ $errors->first('roles') ? 'is-invalid' : '' }}" id="CUSTOMER" value="CUSTOMER">
